...
- Geminiへの指摘・フィードバックを受けたら、再発防止に必要な内容をこの `GEMINI.md` に反映してください。全エージェントに関係する内容は `AGENTS.md`、`CLAUDE.md`、`GEMINI.md` の整合性を保ってください。
- Gemini向けの具体的な繰り返しタスクは、必要に応じてGemini固有の手順書やSkillに分離してください。
