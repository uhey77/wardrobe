...
- Claudeへの指摘・フィードバックを受けたら、再発防止に必要な内容をこの `CLAUDE.md` に反映してください。全エージェントに関係する内容は `AGENTS.md`、`CLAUDE.md`、`GEMINI.md` の整合性を保ってください。
- Claude向けの具体的な繰り返しタスクは、必要に応じて `.claude/skills/<名前>/SKILL.md` に Skill として定義することを検討してください。
